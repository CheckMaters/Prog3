...
int service_func (int * sock_fd, char * account_name_char) {
	...
	bank_accounts * this_account = malloc(sizeof(bank_accounts));
    ...
    printf("In session with this account.\n\n");
...
int add_account (char * new_account) {
	...
	bank_accounts * ptr = malloc(sizeof(bank_accounts));
	ptr = account_list;
	bank_accounts * ptr2 = NULL;
	printf("We are in add_account - %s\n\n", new_account);
	if (ptr == NULL) {
        	//ptr->username = (char *) malloc (sizeof(char) * (strlen(new_account) + 1));
		strcpy(ptr->username, new_account);
		ptr->balance = 0;
		ptr->session_flag = -1;
		ptr->next = NULL;
		account_list = ptr;
	...
	bank_accounts * temp = malloc(sizeof(bank_accounts));
	//temp->username = (char *) malloc (sizeof(char) * (strlen(new_account) + 1));
	strcpy(temp->username, new_account);
	temp->balance = 0;
	temp->next = NULL;
	temp->session_flag = -1;
