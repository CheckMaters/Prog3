...
void timer_handler (int signum) {
	...
	bank_accounts * printer = malloc(sizeof(bank_accounts));
	printer = account_list;
	char* printuser = malloc(sizeof(char)*255);
	printf("\nCurrent bank account list:\n\n");
	//bzero(printuser, sizeof(printuser));
	while(printer != NULL) {
		bzero(printuser, sizeof(printuser));
		/*
		int k;
		...
		for(k = 0; k < strlen(printer->username); k++){
			if(printer->username[k] == '\n'){
				lastspace2 = 1;
			}
		}
		...
		if(lastspace2 == 1){
			printf("got in\n");
			for(k=0; k < strlen(printer->username)-1; k++){
				printuser[k]=printer->username[k];
			}
		}
		*/
	
		if(strlen(printer->username) < 255) {
			strncpy(printuser, printer->username, (strlen(printer->username) - 1));
		} else {
			if(printer->username[254] == '\n'){
				strncpy(printuser, printer->username, 254);
			} else {
				strcpy(printuser, printer->username);
			}
		}
		
			
		if(printer->session_flag == 0){
		
			printf("%s	", printuser);
			printf("%f	%s\n",printer->balance, "In Service");
		} else {
...
int main () {
	...
	//int opt = 1;
	pthread_t thread2;
	pthread_create(&thread2, NULL, &timer,NULL);
//opening socket
...
void * connection_handler (void * socket_fd) {
	...
	printf("\n\t*\tClient's connection is accepted by the Server\t*\n");
