...
void * first_thread (void * args){
	...
	char buffer_to_send[257];          //this will be the modified buffer that we are going to send to the server
	char buffer[280];                  //this is the buffer we are going to read from the STDIN
	...
	while(1){
		...
		if(input_handler_result == 1){
            ...
            //user wants to start a session, which will be handled by service_func
        ...
        //sincer user isn't in session yet, he/she can't ask for deposit, withdraw, query or end
        ...
        //other actions which aren't allowed
		else {
...
void print_info(){
	...
	printf("To start a session with your existing account, TYPE IN :- serve <accountname>\n");
...
int service_func (int * socket_fd) {
	...
	while (1) {
		print_service_info();         //printing the activites user can perform in session
		...
		if(input_handler_result == 6) {	
            ...
            service_mode = 0;   //because this useris ending the session with his/her account
			return 1;
		}
